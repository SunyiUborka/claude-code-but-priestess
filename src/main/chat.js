...
function sendProactive(opts) {
  ...
  // Its own throwaway session: a check runs up to the daily cap times a day and carries a
  // screenshot, so resuming the Doctor's session would leave every one of those
  // images and prompts in it for good, re-sent on every later turn.
  // Cleared, not assigned — activeSessionIds() builds the namespace, and a
  // leftover one from an earlier turn must not be resumed either.
  ephemeralSessionIds = null;
...
function sendMaintenance() {
  ...
  // Its own throwaway session: the memory pass rewrites MEMORY.md and says nothing; none of that
  // belongs in the conversation's session.
  // Cleared, not assigned — activeSessionIds() builds the namespace, and a
  // leftover one from an earlier turn must not be resumed either.
  ephemeralSessionIds = null;
